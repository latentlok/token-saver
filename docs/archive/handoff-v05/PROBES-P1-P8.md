...
**RESULT (2026-07-29): field names observed live — NO CHANGE.** Two `scoped` runs
(`rdab235`, `r7f320f`), each a targeted edit to an existing function under a real gate;
both `STATUS: success`, 1/3 attempts, `writes_attributed: 1` (matching the one changed
file), `denials: 0`. The second run captured the raw `tool_input` keys via a temporary
(reverted) debug append in `scoped_hook.py:main()` — the live executor's calls were:
`read_file|['file_path']` and `edit|['file_path','old_string','new_string']`.
So the executor's edit tool is named `edit` (in the named write-branch
`("write_file","edit","replace")`, `scoped_hook.py:131`) and carries `file_path`, which
that branch reads at `:132` → the write is attributed. The content fields are
`old_string`/`new_string`, NOT `content` — but that divergence is **immaterial**: `edit`
is a *named* write tool, so it never reaches the `EFFECT_KEYS` unknown-tool check at
`:143`, and the named branch attributes by PATH (`file_path`), not content. There is no
write-tool field the hook fails to read that leaves a write unattributed — the hole P3
hunts does not exist on this executor. `EFFECT_KEYS` left unchanged. Caveat: one executor
(`qwen-local`) on a trivial single-file edit; a different executor naming its edit tool
outside `write_file`/`edit`/`replace` (e.g. a `str_replace_editor`) WOULD fall through to
the `:143` shape-check — re-probe if a second executor is wired in.
...
**RESULT (2026-07-29): MIXED — keep opt-in, do NOT flip default-on. Check gap found,
not worker recalcitrance.** Two runs, both `fixture_provenance: true`, real gate:

- **P7a — JSON fixture (comment-free), run `r1f4f1d`:** `STATUS: fixture_unproven`, 3/3
  attempts, every attempt "testdata/users.json lack captured-from: provenance." The worker
  DID comply in spirit: it wrote `testdata/users.json.src` containing
  `captured-from: generated by assistant on 2026-07-29 for parse_users fixture` (an honest
  generated-source disclosure — the instruction's "if you generated the data, say so").
  But `_unproven_fixtures` (`engine.py:251-292`) honors the `.src` sidecar ONLY for binary
  files (`if b"\0" in head`, `:271`); for text files it checks the file's OWN first 10 lines
  (`:285-291`) and never consults the sidecar. JSON has no comments, so a `captured-from:`
  line cannot be prepended without breaking the format — there was NO compliant route for
  this fixture. The worker's `--- qwen result ---` ("provenance is already in place") was
  the worker's claim; the gate said `fixture_unproven` — per the rules, the worker did not
  comply in a form the check accepts.
- **P7b — Python fixture (comment-friendly), run `r52c4c9`:** `STATUS: success`, 2/3
  attempts. Attempt 1 created the fixture without a header (fed back); attempt 2 added
  `# captured-from: generated-by-assistant 2026-07-29 (...)` as the first line of
  `testdata/config_fixture.py` → check passed. The worker complies within the attempt
  budget WHEN a compliant in-file route exists.

**Isolated finding:** the worker is compliant (P7b, attempt 2); the P7a thrash is a check
gap — comment-free text formats (JSON, strict CSV, anything where a header line breaks
parsing) have no compliant path because the sidecar route is binary-only. Default-on would
thrash the common JSON-fixture case to `fixture_unproven`, so the compliance friction IS
real → **keep opt-in.** No code/spec change, no commit (record-only).

**Pinpointed fix (PENDING, not a v0.5 blocker — post-release or a quick v0.5.1):** drop the
binary-only gate on the sidecar. In `_unproven_fixtures` (`engine.py:271-281`), honor
`<path>.src` for a text file too when the file's own first 10 lines lack the header — the
binary/text split exists because "binary files cannot carry the header themselves," but
comment-free text formats effectively cannot either. With that fix, re-run P7a: if the
worker's existing sidecar clears the check → default-on becomes safe to flip (mirror P1's
pattern: `args.get("fixture_provenance", True)` at `engine.py:848`, harness opt-out via
`QWEN_DELEGATE_CONFIG`, a `test_default_on`, reword `schemas.py:128`, note in `docs/HLD.md`).
The feedback prompt (`engine.py:1399-1410`) should also say the sidecar is for ANY file
that can't carry an in-file header, not just "a BINARY fixture."
