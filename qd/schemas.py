...
TOOL = json.loads(r'''
{
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "trust": {
        "type": "string",
        "description": "'self': L5 full trust -- the delegate writes AND grades its own suite; `verify` optional (server generates a non-vacuous-suite gate). 'verified': your `verify` command is the gate -- pass it for stakes you must know rather than trust. Omit to use the project's slider position (`trust` in .qwen-delegate.json), else the built-in default 'self'. Intermediate levels: parked."
      },
      "touch_scope": {
        "type": "array",
        "items": {
