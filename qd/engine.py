...
#
# NINE keys, not the seven declared in this file: `_brief` (set at
...
# U7, same convention: "the endpoint probe already ran for this call". Set by
# qd/server.py's _endpoint_refusal after it probes each DISTINCT endpoint of a
# chain/batch once at the head -- without it every link and item would probe
# again, turning one GET per call into one per item (and on a chain, re-asking
# a question whose answer cannot change mid-call: the links run back-to-back).
# Forgeable from the wire like the rest of the family (see the note above); a
# forged value skips one liveness check and the run then fails exactly the way
# it did before the probe existed, so the lie buys nothing.
PROBED_ARG = "_endpoint_probed"
...
def _delegate(args, t0_dir):
    ...
    # U7: is the endpoint provably there AT ALL, before anything is paid for
    # it. Field 2026-08-07: a hard-down endpoint (502 in ~8ms) cost two runs
    # ~380s of API errors, 0 tokens, both filed gate_suspect -- the outage
    # billed as the worker's failure. Placed after resolve() because the URL
    # lives on the resolved profile, and BEFORE the worktree/T0/pre-flight
    # below: a run that cannot spawn its worker must not acquire a container
    # or pay a gate run that can cost the whole verify budget. Skipped when
    # the server already probed this call's endpoints (PROBED_ARG -- once per
    # call, not per chain link or batch item); never cached across calls, so
    # a just-restarted endpoint works on the next one. Profiles with no
    # discoverable base URL are never probed (see qd/probe.py).
    if not args.get(PROBED_ARG):
        _unreachable = probe.refusal(profile)
        if _unreachable:
            return _refusal(_unreachable, max_iter)
